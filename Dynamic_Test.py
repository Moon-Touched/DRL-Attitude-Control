...
class SimManager(sysModel.SysModel):
    ...
    def UpdateState(self, CurrentSimNanos):
        ...
        if CurrentSimNanos < macros.sec2nano(1):
            rwAvaiOutMsgBuffer.wheelAvailability = [messaging.AVAILABLE, messaging.AVAILABLE, messaging.AVAILABLE, messaging.AVAILABLE]

        else:
            if self.wheelcode == 0:
                rwAvaiOutMsgBuffer.wheelAvailability = [messaging.UNAVAILABLE, messaging.AVAILABLE, messaging.AVAILABLE, messaging.AVAILABLE]
            elif self.wheelcode == 1:
                rwAvaiOutMsgBuffer.wheelAvailability = [messaging.AVAILABLE, messaging.UNAVAILABLE, messaging.AVAILABLE, messaging.AVAILABLE]
            elif self.wheelcode == 2:
                rwAvaiOutMsgBuffer.wheelAvailability = [messaging.AVAILABLE, messaging.AVAILABLE, messaging.UNAVAILABLE, messaging.AVAILABLE]
            elif self.wheelcode == 3:
                rwAvaiOutMsgBuffer.wheelAvailability = [messaging.AVAILABLE, messaging.AVAILABLE, messaging.AVAILABLE, messaging.UNAVAILABLE]
...
class Controller(sysModel.SysModel):
    ...
    def UpdateState(self, CurrentSimNanos):
        ...
        wheelTorqueOutMsgBuffer.motorTorque = [0, 0, 0, 0]
...
def SatelliteRotation(axis, totalTime, timestep):
    ...
    extTorqueOutMsgBuffer = messaging.CmdTorqueBodyMsgPayload()
    if axis == "x":
        extTorqueOutMsgBuffer.torqueRequestBody = [0.01, 0, 0]
    elif axis == "y":
        extTorqueOutMsgBuffer.torqueRequestBody = [0, 0.01, 0]
    elif axis == "z":
        extTorqueOutMsgBuffer.torqueRequestBody = [0, 0, 0.01]
...
def TestWheelFault(wheelcode, attref, totalTime, timestep):
    # region Simulation Parameters
    simTaskName = "simTask"
    simProcessName = "simProcess"
    scSim = SimulationBaseClass.SimBaseClass()
    ...
    # Sim Manager
    simManager = SimManager(wheelCode=wheelcode)
    ...
    return stateLog, rwMotorLog


def myPlot(axis, stateLog, rwMotorLog):
    y = 0
    if axis == "x":
        y = 0
    elif axis == "y":
        ...
        y = 3
    for idx in range(3):
        axes[0, y].plot(timeAxis * macros.NANO2SEC, stateLog.sigma_BN[:, idx], color=unitTestSupport.getLineColor(idx, 3), label=r"$\sigma_" + str(idx + 1) + "$")
    axes[0, y].legend(loc="lower right")
    axes[0, y].set_xlabel("Time [s]")
    axes[0, y].set_ylabel("Orientation MRP")
    axes[0, y].set_title(f"{axis} rotation")
    for idx in range(3):
        axes[1, y].plot(timeAxis * macros.NANO2SEC, stateLog.omega_BN_B[:, idx], color=unitTestSupport.getLineColor(idx, 3), label=r"$\omega_" + str(idx + 1) + "$")
    axes[1, y].legend(loc="lower right")
    axes[1, y].set_xlabel("Time [s]")
    axes[1, y].set_ylabel("angular velocity [r/s]")
    for idx in range(4):
        axes[2, y].plot(timeAxis * macros.NANO2SEC, rwMotorLog.motorTorque[:, idx], label=r"$T_" + str(idx + 1) + "$")
    axes[2, y].legend(loc="lower right")
    axes[2, y].set_xlabel("Time [s]")
    axes[2, y].set_ylabel("Wheel torques [Nm]")
    return


def myPlotwheel(wheelcode, stateLog, rwMotorLog):
    y = wheelcode - 3
    for idx in range(3):
        axes[0, y].plot(timeAxis * macros.NANO2SEC, stateLog.sigma_BN[:, idx], color=unitTestSupport.getLineColor(idx, 3), label=r"$\sigma_" + str(idx + 1) + "$")
    axes[0, y].legend(loc="lower right")
    axes[0, y].set_xlabel("Time [s]")
    axes[0, y].set_ylabel("Orientation MRP")
    axes[0, y].set_title(f"Wheel {wheelcode} fault")
    for idx in range(3):
        axes[1, y].plot(timeAxis * macros.NANO2SEC, stateLog.omega_BN_B[:, idx], color=unitTestSupport.getLineColor(idx, 3), label=r"$\omega_" + str(idx + 1) + "$")
    axes[1, y].legend(loc="lower right")
    axes[1, y].set_xlabel("Time [s]")
...
# axis = "x"
# stateLog, rwLog = TestWheelFault(0, [-0.5, -0.5, -0.5], 15, 0.01)
# timeAxis = stateLog.times()
# myPlotwheel(1, stateLog, rwLog)
# axis = "y"
# stateLog, rwLog = TestWheelFault(1, [-0.5, -0.5, -0.5], 15, 0.01)
# myPlotwheel(2, stateLog, rwLog)


axis = "z"
stateLog, rwLog = TestWheelFault(2, [-0.5, -0.5, -0.5], 15, 0.01)
timeAxis = stateLog.times()
myPlotwheel(3, stateLog, rwLog)
axis = "w"
stateLog, rwLog = TestWheelFault(3, [-0.5, -0.5, -0.5], 15, 0.01)
myPlotwheel(4, stateLog, rwLog)
